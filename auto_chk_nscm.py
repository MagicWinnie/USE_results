...
prev_df_nscm = pd.DataFrame()
_, prev_df_nscm, _ = nscm.get_updates(prev_df_nscm)

print('[' + datetime.now().strftime("%H:%M:%S") + ']', "первые 5 результатов на данный момент:")
print(prev_df_nscm.head(5))
...
while True:
    ...
    try:
        flag_nscm, _, diff_nscm = nscm.get_updates(prev_df_nscm)
...
if result == 1:
    ...
    print("Изменения НИМРО (первые 5 строчек):")
    print(diff_nscm.head(5))
elif result == -1:
